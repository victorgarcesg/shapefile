...
export function loadshp(config, returnData) {
    ...
    loadEPSG('//epsg.io/' + EPSG + '.js', function () {
        ...
        if (typeof url != 'string') {
            ...
            reader.onload = async function (e) {
                await JSZip.loadAsync(data).then(zipResult => {
                    readedZip = zipResult;
                });
                ...
                if (prjString) {
                    await zip.file(prjString.name).async("text").then(text => {
                ...
                await zip.file(shpString).async("arraybuffer").then(buffer => {
                    Preprocess.SHPParser.load(URL.createObjectURL(new Blob([buffer])), shpLoader, returnData);
                });
                await zip.file(dbfString).async("arraybuffer").then(buffer => {
                    Preprocess.DBFParser.load(URL.createObjectURL(new Blob([buffer])), encoding, dbfLoader, returnData);
                });
            }
